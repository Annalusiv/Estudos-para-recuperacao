<?php
//Quando esta em uma pasta de arquivo separada, deve ter esse require, se não tiver separação, basta colocar sem caminho
require_once '../Config/Conexao.php';
// verifica se tem envio de formulario
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Verifica se ja ta cadastrado o email
    if (empty($erros)) {
        try {
    ...
    // se não tiver erros, o cadastro segue normalmente
    if (empty($erros)) {
        try {
    ...
    // Se houver erros, redireciona de volta para a página de cadastro com os erros
   if (!empty($erros)) {
        session_start();
